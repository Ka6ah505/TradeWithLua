...
class MoexCandlesLoader:
    ...
    def _get_candles_chunk(self, security, interval, market, board, start, end, count=500):
    ...
    def load_candles(self, security, interval=1, market='shares', board='TQBR', 
        ...
        max_attempts = 100  # Максимальное количество итераций для защиты от бесконечного цикла
        attempt = 0
    ...
    for tiker in tikers:
        try:
            ...
            if os.path.exists(f'download_data/{tiker}/{tiker}_{type_history}.txt'):
                ...
                to_save_df.to_csv(f'download_data/{tiker}/{tiker}_{type_history}.txt', sep=';', index=False)
            else:
                df.to_csv(f'download_data/{tiker}/{tiker}_{type_history}.txt', sep=';', index=False)
